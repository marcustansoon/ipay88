    ...
    public function validateField($field, $data)
    {
        switch ($field)
        {
            ...
            case 'Remark':
                ...
                break;
            case 'Lang':
                if (in_array(strtoupper($data) , array(
                    'ISO-8859-1',
                    'UTF-8',
                    'GB2312',
                    'GD18030',
        ...
        public function getResponse($requery = true, $return_data = true)
        {
            ...
            if ($requery)
            {
                ...
                if ($return['status'])
                {
                    ...
                    // Generate signature based on response
                    $this->generateSignature(array(
                    ...
                    if ($data['_RequeryStatus'] !== '00' && $this->getField('Signature') !== $data['Signature'])
                    {
        ...
        public function generateSignature($signature_params = array())
        {
            ...
            if ($signature_params)
            {
                ...
                foreach ($signature_params as $key => $val)
                {
                    ...
                    {
                        trigger_error('Missing or invalid parameters required for ' . $key . ' with value : ' . $val);
                        return false;
                    }
            ...
            $signature .= $signature_params['PaymentId']??null ? $signature_params['PaymentId'] : '';
            $signature .= $signature_params['RefNo'];
            $signature .= $signature_params['Amount'];
            $signature .= $signature_params['Currency'];
            $signature .= $signature_params['Status']??null ? $signature_params['Status'] : '';
        ...
        public function validateResponse($response)
        {
            ...
            // Compare signature
            if ($this->getField('Signature') !== trim($response['Signature']))
            {
